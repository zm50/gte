...
// Router 任务执行流路由器
type Router struct {
	apis map[uint32]trait.TaskFlow
}
...
func NewRouter() trait.Router {
	return &Router{
		apis: make(map[uint32]trait.TaskFlow),
	}
}

// Regist 注册任务执行逻辑
func (r *Router) Regist(id uint32, flow ...trait.TaskFunc) {
...
func (r *Router) RegistFlow(id uint32, flow trait.TaskFlow) {
	r.apis[id] = flow
}

// TaskFlow 根据消息ID获取任务执行流
func (r *Router) TaskFlow(id uint32) trait.TaskFlow {
